...
export default function Login() {
  ...
  const deltaX = (mousePos.x - centerX) / centerX; // -1 to 1
  const deltaY = (mousePos.y - centerY) / centerY; // -1 to 1

  // Professional parallax is subtle. Reduced from 8deg to 3deg
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (role === 'citizen') {
        res = await authAPI.citizenLogin({ phone: formData.phone, password: formData.password });
      } else if (role === 'contractor') {
        res = await authAPI.contractorLogin({ contractorNumber: formData.contractorNumber, password: formData.password });
      } else {
        res = await authAPI.authorityLogin({ governmentId: formData.governmentId, password: formData.password });
      }
  ...
  const roleConfig = {
    citizen: { label: 'Citizen', emoji: '👤', hint: 'Phone: 9000000001 | Pass: Citizen@123' },
    contractor: { label: 'Contractor', emoji: '🏗️', hint: 'ID: CTR-2026-001 | Pass: Contractor@123' },
    authority: { label: 'Authority', emoji: '🏛️', hint: 'Gov ID: GOV-2026-001 | Pass: Authority@123' },
  };
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif', background: '#020617', alignItems: 'center', justifyContent: 'center', padding: '20px', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Decorative background mesh - Professional deep tones */}
      ...
      {/* Cursor tracking spotlight - Soft ambient glow */}
      <div style={{ 
        position: 'absolute', 
        top: 0, left: 0, 
      ...
      <div style={{
        ...
        width: '100%',
        maxWidth: '440px',
        background: 'rgba(15, 23, 42, 0.4)', // Darker, cleaner slate background
        backdropFilter: 'blur(32px)',
        WebkitBackdropFilter: 'blur(32px)',
        border: '1px solid rgba(255, 255, 255, 0.08)', // Finer, subtler border
        borderRadius: '16px', // Slightly less rounded for a professional look
        padding: '48px 40px',
        ...
        <h2 style={{ fontSize: '1.6rem', fontWeight: '700', letterSpacing: '-0.5px', color: 'white', marginBottom: '6px', textAlign: 'center' }}>Welcome back</h2>
        <p style={{ color: '#94a3b8', marginBottom: '32px', textAlign: 'center', fontSize: '0.9rem' }}>Sign in to your account to continue</p>

        {/* Role Tabs */}
        <div style={{ display: 'flex', padding: '4px', background: 'rgba(15, 23, 42, 0.6)', borderRadius: '10px', marginBottom: '32px', border: '1px solid rgba(255,255,255,0.05)' }}>
          {Object.entries(roleConfig).map(([r, cfg]) => (
            <button
              ...
              style={{
                ...
                boxShadow: role === r ? '0 2px 4px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.1)' : 'none',
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          {role === 'contractor' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontWeight: '500', fontSize: '0.85rem', color: '#e2e8f0' }}>Contractor ID</label>
              <input
                type="text"
                className="glass-input"
          ...
          {role === 'authority' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontWeight: '500', fontSize: '0.85rem', color: '#e2e8f0' }}>Government ID</label>
              <input
                type="text"
                className="glass-input"
          ...
          <div style={{
            ...
            <strong style={{ fontWeight: '600', color: '#cbd5e1' }}>Demo Access:</strong><br/>
